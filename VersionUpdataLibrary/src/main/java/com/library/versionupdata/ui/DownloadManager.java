...
import com.library.versionupdata.callback.DownloadListener;
import com.library.versionupdata.http.HttpHelper;
import com.library.versionupdata.http.FileCallBack;
import com.library.versionupdata.utils.ALog;
import com.library.versionupdata.utils.AppUtils;
import com.library.versionupdata.utils.VersionFileProvider;
...
public class DownloadManager {
    ...
    public static void downloadAPK(final String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        HttpHelper.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), AllenChecker.getGlobalContext().getString(R.string.versionchecklib_download_apkname, AllenChecker.getGlobalContext().getPackageName())) {
            @Override
...
//                showFailDialog();
    ...
    private static void silentDownloadAPK(final Context context, String url, final VersionParams versionParams, final DownloadListener listener) {
        ...
        if (listener != null)
            listener.onCheckerStartDownload();
        HttpHelper.getHttpClient().newCall(request).enqueue(new FileCallBack(versionParams.getDownloadAPKPath(), context.getString(R.string.versionchecklib_download_apkname, context.getPackageName())) {
